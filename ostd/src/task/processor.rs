...
use crate::{arch::task, cpu_local_cell, irq::DisabledLocalIrqGuard};
...
pub(super) fn switch_to_task(next_task: Arc<Task>) {
    ...
    let current_task_ctx_ptr = if !current_task_ptr.is_null() {
        ...
        unsafe { task::first_context_switch(next_task_ctx_ptr) };
    ...
    unsafe {
        ...
        #[cfg(not(miri))]
        task::context_switch(next_task_ctx_ptr, current_task_ctx_ptr);
        #[cfg(miri)]
        crate::task::scheduler::miri_switch_to(miri_next_task.as_ref());
    }
